...
(function() {
  ...
  function switchPage(pageId) {
    // Update nav highlighting with minimal reflows
    ...
    // Render charts only once when needed
  ...
  function renderCharts() {
    ...
    if (trafficCanvas && !chartInstances.traffic) {
      try {
    ...
    if (bounceCanvas && !chartInstances.bounce) {
      try {
        const ctx = bounceCanvas.getContext('2d');
        chartInstances.bounce = new Chart(ctx, {
          type: 'bar',
          data: {
            labels: ['Jan', 'Feb', 'Mar', 'Apr', 'May'],
            datasets: [{
        ...
        console.error('Error rendering bounce chart:', error);
      }
    }
  ...
  // Debounce window resize for chart responsiveness
  let resizeTimer;
  window.addEventListener('resize', () => {
    clearTimeout(resizeTimer);
    resizeTimer = setTimeout(() => {
      if (chartInstances.traffic) chartInstances.traffic.resize();
      if (chartInstances.bounce) chartInstances.bounce.resize();
    }, 250);
  });
  ...
  window.addEventListener('beforeunload', () => {
    ...
    if (chartInstances.bounce) chartInstances.bounce.destroy();
  });
});
